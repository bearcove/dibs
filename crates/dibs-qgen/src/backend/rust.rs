...
use dibs_query_ir::{
    ApiOperationName, CompiledQuery, CompiledQueryError, FieldId, OrderedBind, OutputField,
    Parameter, ParameterApiContract, ParameterBindAdapter, ParameterId, ParameterPassing,
    ResultMode, RuntimeAssertion, TargetLanguage,
};
...
pub fn generate_compiled_rust(query: &CompiledQuery) -> Result<GeneratedRust, RustGenerationError> {
    ...
    let result_name = result_type_name(&query.query_name);
...
fn return_type(mode: ResultMode, result_name: &str) -> String {
    match mode {
        ResultMode::Many => format!("QueryResult<Vec<{result_name}>>"),
        ResultMode::Optional => format!("QueryResult<Option<{result_name}>>"),
        ResultMode::One => format!("QueryResult<{result_name}>"),
...
fn result_type_name(query_name: &str) -> String {
    format!("{query_name}Result")
}
