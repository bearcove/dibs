...
#[tokio::test]
async fn postgres_18_accepts_compiled_full_language_sql() {
    let (_container, client) = setup_postgres().await;
    client
        .batch_execute(
            "create table run (id bigint primary key); insert into run values (1), (2), (3)",
        )
        .await
    ...
    for (index, (name, source, expected)) in cases.into_iter().enumerate() {
        let compiled =
            compile_query_source(&parser, SourceId::new(140 + index as u32), source, &catalog)
                .unwrap_or_else(|error| panic!("{name} oracle query compiles: {error:?}"));
        let query = compiled[0]
            .validate()
            .unwrap_or_else(|error| panic!("{name} oracle artifact validates: {error}"));
        let rendered = render_compiled_sql(query)
            .unwrap_or_else(|error| panic!("{name} oracle SQL renders: {error}"));
        let rows = client
            .query(&rendered.sql, &[])
            .await
        ...
        let actual = rows
            ...
            .map(|row| {
                (0..row.len())
                    .map(|column| postgres_cell_text(row, column))
                    .collect::<Vec<_>>()
                    .join("|")
            })
            .collect::<Vec<_>>()
...
fn postgres_cell_text(row: &Row, column: usize) -> String {
    let ty = row.columns()[column].type_();
    if *ty == tokio_postgres::types::Type::INT8 {
        row.get::<_, i64>(column).to_string()
    } else if *ty == tokio_postgres::types::Type::INT8_ARRAY {
        let values = row.get::<_, Vec<i64>>(column);
        format!(
            "{{{}}}",
            values
                .iter()
        ...
        panic!("unsupported oracle result type {ty}")
    }
}
